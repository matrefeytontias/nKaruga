		...
		if(KFIRE(kEv))
		{
			if(!fireDelay)
			{
				...
				if(fireRepeat)
				{
					...
					Level::bArray->addRadial(x - itofix(img[0][0]) / 4, y, 192, itofix(6), LUTs::BaseImageId::PLAYER_BULLET_LIGHT, polarity, false, Constants::CamRelation::DEFAULT);
					Level::bArray->addRadial(x + itofix(img[0][0]) / 4, y, 192, itofix(6), LUTs::BaseImageId::PLAYER_BULLET_LIGHT, polarity, false, Constants::CamRelation::DEFAULT);
					fireDelay = 4;
				...
				{
					// fire 1 bullet
					Level::bArray->addRadial(x, y, 192, itofix(6), LUTs::BaseImageId::PLAYER_BULLET_LIGHT, polarity, false, Constants::CamRelation::DEFAULT);
					fireDelay = 8;
					fireRepeat = true;
					for(int i = 0; i < 8; i++)

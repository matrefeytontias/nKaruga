...
void cb_Pattern_1_6(cb_args)
{
	...
	if (!(e->internal[0] % 25))
	{
		Level::bArray->add(e->getx(), e->gety(), angle, itofix(1), static_cast<int>(LUTs::BaseImageId::ENEMY_BULLET_1_LIGHT), e->polarity, true, e->getCamRelation());
	}
	e->internal[0]++;
	...
	if (!(Level::waveTimer % 2))
	{
		...
		if (e->internal[0] > 65 && e->internal[0] < 100)
		{
			if (!(Level::waveTimer % 8))
				Level::bArray->add(e->getx(), e->gety(), angle, itofix(2), static_cast<int>(LUTs::BaseImageId::ENEMY_BULLET_1_LIGHT), e->polarity, true, e->getCamRelation());
		}
...
void cb_Pattern_1_9(cb_args)
{
	...
	if (!(Level::waveTimer % 4))
		Level::bArray->add(e->getx(), e->gety(), 64, itofix(4), static_cast<int>(LUTs::BaseImageId::ENEMY_BULLET_1_LIGHT), e->polarity, true, e->getCamRelation());
}
	...
	if (!(Level::waveTimer % 2))
	{
		...
		else if (e->internal[0] - 45 < 416)
		{
			...
			if (e->internal[4] >= 32)
			{
				...
				{
					angle = e->angleToEntity(Level::p);
					Level::bArray->add(e->getx(), e->gety(), angle, itofix(2), static_cast<int>(LUTs::BaseImageId::ENEMY_BULLET_0_LIGHT), e->polarity, true, e->getCamRelation());
				}
				e->internal[1] += 4;
			}
		}
			...
			if (!(Level::waveTimer % 2))
			{
				for (int i = 0; i < 4; i++)
				{
					...
					Level::bArray->add(fixcos(k + (e->internal[0] - 3) * 8) * e->img[0] / 2 + e->getx(), fixsin(k + (e->internal[0] - 3) * 8) * e->img[1] / 2 + e->gety(),
						k, (e->internal[0] - 2) * 64 + itofix(1), static_cast<int>(LUTs::BaseImageId::ENEMY_BULLET_0_LIGHT), e->polarity, true, e->getCamRelation());
				}
				e->internal[0]++;
			}
		}
					...
					for (int i = 0; i < 5; i++)
					{
						...
						for (int j = 0; j < 3; j++)
						{
							...
							Level::bArray->add(fixcos(k) * e->img[0] / 2 + e->getx() - fixsin(k) * (j - 1) * 6, fixsin(k) * e->img[1] / 2 + e->gety() + fixcos(k) * (j - 1) * 6,
								k, itofix(2), static_cast<int>(LUTs::BaseImageId::ENEMY_BULLET_2_LIGHT), e->polarity, true, e->getCamRelation());
						}
					}
				}
				e->internal[0]++;
...
void cb_Pattern_1_15(cb_args)
{
	...
	if (!(Level::waveTimer % 2))
	{
		...
		{
			angle = e->angleToEntity(Level::p);
			Level::bArray->add(e->getx(), e->gety(), angle, itofix(1), static_cast<int>(LUTs::BaseImageId::ENEMY_BULLET_0_LIGHT), e->polarity, true, e->getCamRelation());
		}
		e->internal[0]++;
		e->rotationAngle = e->internal[0];
	}
...
void cb_Pattern_1_16(cb_args)
{
	if (e->gety() <= itofix(80))
	{
		if (!(Level::waveTimer % 16) && e->gety() < itofix(60))
			Level::bArray->add(e->getx(), e->gety(), 64, itofix(3), static_cast<int>(LUTs::BaseImageId::ENEMY_BULLET_1_LIGHT), e->polarity, true, e->getCamRelation());
...
void cb_Pattern_1_20(cb_args)
{
	if (!(Level::waveTimer % 16))
		Level::bArray->add(e->getx(), e->gety(), (e->waveIndex + 1 % 2) * 128, itofix(2), static_cast<int>(LUTs::BaseImageId::ENEMY_BULLET_2_LIGHT), e->polarity, true, e->getCamRelation());
}
...
void cb_Pattern_1_bossGrenade(cb_args)
{
	...
	{
		for (int i = 0; i < 32; i++)
			Level::bArray->add(e->getx(), e->gety(), i * 8, itofix(2), static_cast<int>(LUTs::BaseImageId::ENEMY_BULLET_0_LIGHT), e->polarity, true, e->getCamRelation());
...
void cb_Pattern_2_15(cb_args)
{
	...
	e->beAbox();
	if (!(Level::waveTimer % 16))
		Level::bArray->add(e->getx(), e->gety() + itofix(24), 64, itofix(1), static_cast<int>(LUTs::BaseImageId::ENEMY_BULLET_1_LIGHT), !e->polarity, true, e->getCamRelation());
}

void cb_Pattern_2_16(cb_args)
{
	if (!(Level::waveTimer % (fixtoi(fixdiv(itofix(40), 256 - Constants::CAMERA_SPEED * 2)))))
	{
		if (e->internal[0] % 2)
		{
	...
	if (fToScreenY(e->gety(), e->getCamRelation()) > 0) e->deactivate(); // stop spawning boxes when the spawnpoint is visible
}
...
void cb_Pattern_2_20(cb_args)
{
	...
	{
		e->rotationAngle = 0;
		e->addy(96 + Constants::CAMERA_SPEED * 3);
	}
}
...
void cb_Pattern_2_21(cb_args)
{
	...
	if (!(Level::waveTimer % 6))
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			Level::bArray->add(e->getx() + temp.x, e->gety() + temp.y, -(e->rotationAngle + i * 64), 256, static_cast<int>(LUTs::BaseImageId::ENEMY_BULLET_0_LIGHT), i % 2 ? Constants::SHADOW : Constants::LIGHT, true, e->getCamRelation());
		}
	}
	e->beAprop();
}
...
void cb_Pattern_2_23(cb_args)
{
	...
	e->sety((fixsin(Level::waveTimer) / 25 + 30) * fixsin(Level::waveTimer +(e->waveIndex % 6) * 42) + e->internal[0] * Constants::CAMERA_SPEED);
	e->rotationAngle++;
}
...
void cb_Pattern_2_25(cb_args)
{
	...
	else if (e->internal[1] < Constants::INV_CAMERA_SPEED * 40 * 3) // keep firing until 3 boxes pass by
		e->internal[1]++;
	else
	{
		Level::bArray->stop_laser(e);
...
void cb_Pattern_2_29(cb_args)
{
	...
	if (!(Level::waveTimer % 6))
	{
		...
		{
			rotate(itofix(24), 0, 0, 0, e->rotationAngle + i * 64, &temp);
			Level::bArray->add(e->getx() + temp.x, e->gety() + temp.y, -(e->rotationAngle + i * 64), 256, static_cast<int>(LUTs::BaseImageId::ENEMY_BULLET_0_LIGHT), e->polarity, true, e->getCamRelation());
		}
	}
	e->beAprop();
}
...
void cb_Pattern_2_34(cb_args)
{
	...
	if (fixtoi(fToScreenY(e->internal[1], e->getCamRelation())) < 120)
		e->internal[1] += Constants::CAMERA_SPEED;
...
void cb_Pattern_2_36(cb_args)
{
	...
	if (!(t & 3) && t / 4 < 3)
		Level::bArray->add(e->getx(), e->gety(), 64, itofix(2), static_cast<int>(LUTs::BaseImageId::ENEMY_BULLET_1_LIGHT), e->polarity, true, e->getCamRelation());
}

void cb_Pattern_2_wall(cb_args)
{
	if (fixtoi(fToScreenY(e->gety(), e->getCamRelation())) >= 120)
	{

...
void cb_Pattern_2_18(cb_args)
{
	...
	if (!(Level::waveTimer % 30))
		Level::enemiesArray->add(fToScreenX(e->getx(), e->getCamRel()), fToScreenY(e->gety(), e->getCamRel()), 1, static_cast<int>(LUTs::BaseImageId::ENEMY_SHIP_1_SHADOW), static_cast<int>(LUTs::EnemyPatternId::C2_P20), 0, e->polarity, true, 2, false, static_cast<int>(Constants::EnemyType::ENEMY));
	e->beAprop();
}
...
void cb_Pattern_2_19(cb_args)
{
	if (!((Level::waveTimer + 15) % 30))
		Level::enemiesArray->add(fToScreenX(e->getx(), e->getCamRel()), fToScreenY(e->gety(), e->getCamRel()), 1, static_cast<int>(LUTs::BaseImageId::ENEMY_SHIP_1_LIGHT), static_cast<int>(LUTs::EnemyPatternId::C2_P20), 0, e->polarity, true, 2, false, static_cast<int>(Constants::EnemyType::ENEMY));
	e->beAprop();
}

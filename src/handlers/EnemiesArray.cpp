...
Enemy* EnemiesArray::add(int x, int y, int HP, LUTs::BaseImageId shipImgId, int patternId, int waveIndex, bool polarity, bool hasRotation, int firebackAmount, bool ghost, int type)
{
	// Keep props active
	...
	data[currentEnemy].activate(x, y, HP, shipImgId, static_cast<LUTs::EnemyPatternId>(patternId), waveIndex, polarity, hasRotation, firebackAmount, ghost, static_cast<Constants::EnemyType>(type));
